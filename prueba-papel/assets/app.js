...
function renderHome(){
  $("#content").innerHTML=`
    <section class="hero">
      <div class="kicker" style="color:#fff;opacity:.75">PRUEBA DE PAPEL · B26</div>
...
function exportProgress(){
  const blob=new Blob([JSON.stringify({format:"B26_PAPEL_V1",exportedAt:new Date().toISOString(),state:state()},null,2)],{type:"application/json"});
  const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="progreso_prueba_papel_b26.json";a.click();URL.revokeObjectURL(a.href);
}
function resetProgress(){
  if(!confirm("¿Restablecer todo el progreso de Prueba de Papel?"))return;
