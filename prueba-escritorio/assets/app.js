...
const KEY="b26_escritorio_v5";
const LEGACY_KEY="b26_escritorio_v4";
function state(){
  try{
...
function renderExercise(ex){
  ...
  <div class="actions"><button class="btn primary" id="checkBtn">✓ Comprobar</button><button class="btn soft" id="exampleInlineBtn">📘 Ejemplo similar resuelto</button><button class="btn" id="hintBtn">💡 Pista</button><button class="btn" id="stepBtn">👣 Paso a paso</button><button class="btn" id="clearBtn">Limpiar</button><button class="btn good" id="nextBtn">Siguiente ejercicio →</button></div>
  <div id="exampleInline" class="example-inline">${examplePanelHtml(ex)}</div><div id="hint" class="hint">${esc(ex.hint)}</div><div id="feedback" class="feedback"></div><div id="stepPanel" class="step-panel"><b>Traza guiada</b><p class="muted">Revela una fila cada vez y compárala con tu razonamiento.</p><div id="stepContent"></div><button class="btn soft" id="revealBtn">Revelar siguiente paso</button></div></section>`;
  ...
  $("#hintBtn").onclick=()=>$("#hint").classList.toggle("show");$("#stepBtn").onclick=()=>$("#stepPanel").classList.toggle("show");$("#exampleInlineBtn").onclick=()=>$("#exampleInline").classList.toggle("show");
